...
import { isLogin, placeholderImage, t, truncate, getCompressedImage, normalizeImageUrl } from "@/utils"
...
const ProfileDropdown = ({ closeDrawer, settings, handleLogout, isDrawer }) => {
    ...
    const handleItemClick = (key) => {
        ...
        const item = items.find(item => item.key === Number(key))
        if (item?.href) {
            router.push(item.href)
        } else {
            console.error("href is undefined for menu item with key: ", key) // Error logging
        }
    }
    ...
    // For mobile/drawer view: render expanded list
    if (isDrawer) {
        return (
            <div className="profile_dropdown_expanded">
                {items.map((item) => (
                    <div
                        key={item.key}
                ...
                {/* Add two empty items for spacing to prevent logout from being hidden by bottom nav */}
                <div className="profile_dropdown_item" style={{ visibility: 'hidden', pointerEvents: 'none' }} aria-hidden="true"></div>
                <div className="profile_dropdown_item" style={{ visibility: 'hidden', pointerEvents: 'none' }} aria-hidden="true"></div>
            </div>
        )
    }

    // For desktop view: render dropdown
    return (
        <Dropdown
            menu={menuProps}
        ...
        >
            <Button className="d-flex align-items-center" id="dropdown-basic">
                {(() => {
                    ...
                    if (!originalProfile) {
                        // No profile image, use placeholder
                        return (
                            <Image
                                loading="lazy"
                    ...
                    // Check if UserData has compressed field with small size
                    const hasCompressed = UserData?.compressed && 
                                         typeof UserData.compressed === 'object' && 
                                         UserData.compressed.small && 
                                         UserData.compressed.small !== '' && 
                                         UserData.compressed.small !== null;
                    
                    // Try to get compressed version
                    const compressedProfile = hasCompressed ? getCompressedImage(UserData, 'small', originalProfile) : null;
                    
                    // If compressed exists and is different from original, use it; otherwise use original
                    const finalProfile = (compressedProfile && compressedProfile !== originalProfile && compressedProfile !== null && compressedProfile !== '') 
                        ? compressedProfile 
                        : originalProfile;
