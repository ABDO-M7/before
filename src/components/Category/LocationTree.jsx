...
const LocationTree = ({
  ...
  // Display name by active language (same as header location)
  const getDisplayName = (node) => {
    if (node?.isLoadMore || node?.key === "all_countries") return node?.title ?? "";
    if (isArabic && node?.name_ar?.trim()) return node.name_ar;
    ...
    return (
      <div className="filter_item_cont">
        {node.isLoadMore ? (
          ...
          <span className={`filter_item`}>{label}</span>
        )}
      </div>
    );
  };
  ...
  const handleLocationSelect = (selectedKeys, info) => {
    ...
    if (selectedKeys.length === 0 || node?.key === "all_countries") {
      ...
      // Pass full node so parent can show name_ar in chip (same as header location)
      if (node?.isCountry) {
        setCountry(node);
      } else if (node?.isState) {
        setState(node);
