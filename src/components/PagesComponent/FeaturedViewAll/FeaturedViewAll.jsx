...
import { getCityData, getKilometerRange } from "@/redux/reuducer/locationSlice";
import { t } from "@/utils";
...
const FeaturedViewAll = ({ slug }) => {
    ...
    const fetchFeaturedSectionData = async () => {
        try {
            const response = await FeaturedSectionApi.getFeaturedSections({ slug: slug, hub: 'web' });
    ...
    const fetchItemsData = async (page) => {
        try {
            ...
            // Prefer most specific location: area -> city -> state -> country
            if (cityData?.areaId) {
            ...
            // Also include radius/lat/long when applicable
            if (KmRange > 0) {
                        ...
                        <div className="row" id='main_row'>
                            <div className="col-12 col-md-6 col-lg-12" id='listing_sec'>
                                ...
                                <div className="listing_items">
                                    <div className={`row ${view === 'grid' ? 'row-cols-xxl-4 row-cols-lg-4 row-cols-md-3 row-cols-2' : ''} product_card_card_gap`}>
                                        {isLoading ?
                                            view === "list" ?
                                            ...
                                            : itemsData && itemsData.length === 0 ?
                                                ...
                                                : view === "list" ?
                                                    itemsData && itemsData.map((ele, index) => (
                                                        <div className="col-12" key={index}>
                                                            <Link onClick={() => handleBreadcrumbPath(ele)} href={ele?.user_id == userData?.id ? `/my-listing/${encodeURIComponent(ele?.slug || '')}` : `/product-details/${encodeURIComponent(ele?.slug || '')}`}>
