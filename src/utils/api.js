...
export const GET_QUICK_SEARCHES = "quick-searches";
...
export const AUTH_BY_TOKEN = "auth-by-token";
...
export const sliderApi = {
    getSlider: ({ hub } = {}) => {
        return Api.get(GET_SLIDER, { params: { hub } });
    },
};

// 3. CATEGORY API
export const categoryApi = {
    getCategory: ({ category_id, page, featured } = {}) => {
        return Api.get(GET_CATEGORIES, {
            params: { category_id, page, featured },
        });
    },
};
// 3. MY ITEMS API
...
export const getBlogsApi = {
    getBlogs: ({ slug, category_id, sort_by, tag, hub, page, limit } = {}) => {
...
// AI TOOLS API
export const getAiToolsApi = {
    getAiTools: ({ slug, sort_by, hub, page, limit, featured } = {}) => {
        return Api.get(GET_AI_TOOLS, {
            params: {
                slug,
                sort_by,
                hub,
...
// Quick Searches API (for "Quick Search" pills under header search)
export const quickSearchesApi = {
    getQuickSearches: (featured = 1) => {
        return Api.get(GET_QUICK_SEARCHES, { params: { featured } });
    },
    getQuickSearchBySlug: (slug) => {
        return Api.get(GET_QUICK_SEARCHES, { params: { slug } });
    },
};
...
export const userSignUpApi = {
    userSignup: ({
        ...
        if (password) formData.append("password", password); // 👈 هيبقى اختياري
...
export const verifyOtpApi = {
    verifyOtp: ({ phone, country_code, otp } = {}) => {
...
export const createPaymentIntentApi = {
    createIntent: ({ package_id, payment_method, platform_type, order_id } = {}) => {
        const formData = new FormData();
    ...
    createIntentWithFormData: (formData) => {
        return Api.post(PAYMENT_INTENT, formData, {
            headers: {
                "Content-Type": "multipart/form-data",
            },
...
export const addItemApi = {
    addItem: ({
        ...
        if (notes != null && notes !== "") formData.append("notes", notes);
