...
export const generateMetadata = async ({ params }) => {
    try {
        ...
        if (!resolvedParams?.slug || !Array.isArray(resolvedParams.slug) || resolvedParams.slug.length === 0) {
            return {
                title: process.env.NEXT_PUBLIC_META_TITLE || 'Category',
...
const SingleCategoryPage = async ({ params }) => {
    ...
    const categoryItems = await getCategoryItems(resolvedParams.slug[0])
    ...
    return (
        <>
            <JsonLd data={jsonLd} />
            <Layout>
                <SingleCategory slug={resolvedParams.slug || []} />
            </Layout>
        </>
    )
}
